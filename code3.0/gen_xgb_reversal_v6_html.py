"""
Generate tradelog.html + trade_replay.html for xauusd_5m_sd_xgb_reversal_v6_2026_04_13
(Strategy Q -- 254% -- metrics.json uses test_metrics key, not test)
"""
...
FOLDER      = "artefacts/approved_strategies/xauusd_5m_sd_xgb_reversal_v6_2026_04_13"
PRICE_CSV   = "data/raw/XAUUSD_1H_2019_2026_full.csv"
LABEL       = "Strategy Q  254% | XGB Reversal + SMC | Single TP | WR=57.9%"
CONTEXT_BARS = 60
...
# ── load trades + metrics ──────────────────────────────────────────────────────
trades_csv   = os.path.join(FOLDER, "trades_test.csv")
...
# V6 uses test_metrics key (auto-generated by pipeline)
tm = metrics.get("test_metrics") or metrics.get("test") or {}
...
# ── replay rows (with candle window indices) ───────────────────────────────────
replay_rows = []
for _, row in df.iterrows():
...
# ── write files ────────────────────────────────────────────────────────────────
log_path    = os.path.join(FOLDER, "tradelog.html")
replay_path = os.path.join(FOLDER, "trade_replay.html")

with open(log_path,    "w", encoding="utf-8") as f:
    f.write(tradelog_html)
print(f"[gen] tradelog.html -> {log_path}")

with open(replay_path, "w", encoding="utf-8") as f:
    f.write(replay_html)
print(f"[gen] trade_replay.html -> {replay_path}")

print(f"\nDone. Trades: {len(df)}, WR: {summary['win_rate']}, Return: {summary['annualized_return']}")
