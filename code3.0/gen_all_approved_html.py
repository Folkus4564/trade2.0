"""
Generate tradelog.html + trade_replay.html for every approved strategy folder.
Output is written directly into each strategy's own folder.
"""
...
# ── label map (folder -> short label for HTML titles) ─────────────────────────
LABELS = {
...
# ── iterate approved strategies ────────────────────────────────────────────────
folders = sorted(os.listdir(APPROVED_DIR))
for folder in folders:
    ...
    if not os.path.exists(trades_csv) or not os.path.exists(metrics_json):
        print(f"  [skip] {folder} -- missing trades_test.csv or metrics.json")
        continue
    ...
    summary = {
        ...
        "annualized_return": f"{tm['annualized_return']*100:.1f}%",
        "sharpe":       f"{tm['sharpe_ratio']:.2f}",
        "max_dd":       f"{tm['max_drawdown']*100:.1f}%",
    ...
    # ── replay trade data (with candle window indices) ──────────────────────────
    replay_rows = []
    for _, row in df.iterrows():
    ...
    replay_summary = {
        "return":   f"{tm['annualized_return']*100:.1f}%",
        "sharpe":   f"{tm['sharpe_ratio']:.2f}",
        "max_dd":   f"{tm['max_drawdown']*100:.1f}%",
        "win_rate": f"{len(wins)/len(df)*100:.1f}%" if len(df) else "N/A",
        "pf":       f"{tm.get('profit_factor', 0):.2f}",
...
showTrade(0);
    ...
    print(f"  tradelog.html    -> {log_path}")
    print(f"  trade_replay.html -> {replay_path}")
    print(f"  Trades: {len(df)} | Months: {len(monthly)}")
