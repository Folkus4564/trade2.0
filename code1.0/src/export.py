"""
Module: export.py
Purpose: Auto-export approved strategies to a dedicated folder for future deployment
...
APPROVED_DIR = ROOT / "approved_strategies"
...
def export_approved_strategy(
    ...
    strategy_name: str = "xauusd_smc_hmm_regime",
) -> Path:
    """
    Export an approved strategy to approved_strategies/<name>_<date>/.

    Creates:
        strategy.py          -- copy of pipeline.py
        config.yaml          -- config snapshot
        metrics.json         -- all split metrics
        training_summary.md  -- auto-generated summary
        model.pkl            -- trained HMM model copy

    Args:
        results:       Full results dict from pipeline
        config:        Config dict snapshot
        model_path:    Path to trained HMM .pkl file
    ...
    export_dir = APPROVED_DIR / f"{strategy_name}_{today}"
    export_dir.mkdir(parents=True, exist_ok=True)

    # ── strategy.py ──────────────────────────────────────────────────────────
    pipeline_src = ROOT / "src" / "pipeline.py"
    if pipeline_src.exists():
        shutil.copy(pipeline_src, export_dir / "strategy.py")
    ...
    if model_path is None:
        model_path = ROOT / "models" / "hmm_regime_model.pkl"
...
def _write_summary(export_dir: Path, results: Dict[str, Any]) -> None:
    """Generate a markdown training summary."""
    test  = results.get("test_metrics", {})
    train = results.get("train_metrics", {})
    val   = results.get("val_metrics",   {})
